        ...
        try
        {
            ...
            String fileExtension =
                fileName.indexOf('.') > 0 ? fileName.substring(
                    fileName.indexOf('.') + 1, fileName.length()) : "jpeg";
...
//                + fileExtension + ")");
            ...
            {
                log.error("No pic directory found and was unable to create: "
            ...
            while (fileOnDisk.exists())
                fileOnDisk =
                    new File(registry.getProjectBaseDirectory() + model
                        + "/pics/" + fileName + "_" + (++index) + "."
                        + fileExtension);
            
            OutputStream fout = new FileOutputStream(fileOnDisk);
            fileName = fileOnDisk.getName();
            registry.getSahaProject(model).addLiteralProperty(
                bean.getResource(), bean.getProperty(), fileName);
...
//            ImageIO.write(image, fileExtension, fout);
            ...
            log.info("Image file " + fileName + " successfully saved to: "
