...
public class ResourceEditService {
	...
	public String getLiteralPropertyEditor(String model, String id, String s, String p, String valueShaHex,
		...
		for (ISahaProperty property : project.getResource(s, locale).getProperties())
			if (property.getValueShaHex().equals(valueShaHex)) modelMap.put("property", property);
	...
	public void removeLiteralProperty(String model, String s, String p, String valueShaHex) {
		try {
			this.sahaProjectRegistry.getLockForProject(model).writeLock().lock();

			IModelEditor project = sahaProjectRegistry.getModelEditor(model);
			project.removeLiteralProperty(s, p, valueShaHex);
		} finally {
			this.sahaProjectRegistry.getLockForProject(model).writeLock().unlock();
		}
	}

	public void removeResource(String model, String uri) {
		try {
			this.sahaProjectRegistry.getLockForProject(model).writeLock().lock();
	...
	public void removeObjectProperty(String model, String s, String p, String o) {
		try {
			this.sahaProjectRegistry.getLockForProject(model).writeLock().lock();
	...
	public void removeProperty(String model, String s, String p) {
		try {
			this.sahaProjectRegistry.getLockForProject(model).writeLock().lock();
	...
	public String getExternalPropertyTable(String ontology, String resourceUri, HttpServletRequest request) {
		...
		modelMap.put("propertyMapEntrySet", onkiWebService.getOnkiRepository(ontology).getPropertyMapEntrySet(resourceUri, locale));
