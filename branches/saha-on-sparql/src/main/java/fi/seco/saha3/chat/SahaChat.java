...
/**
 * Bean that handles the internal project-specific SAHA-chats. Each project has
 * its own chat channel, and admins can broadcast to all channels.
 * 
 */
public class SahaChat implements DisposableBean {
	...
	private class Channel {
		...
		private Channel(String channelName) {
			...
			try {
				...
				log.error("Failed to open log file. Logging to System.out.", e);
			}
		}
	...
	private static final Logger log = LoggerFactory.getLogger(SahaChat.class);
	...
	public synchronized void addMessage(String channelName, String name, String message) {
		...
		if (message != null) {
			...
			if (!message.isEmpty()) if (message.equals("/clear"))
				channel.clear();
	...
	private Channel getChannel(String channelName) {
		if (!channels.containsKey(channelName)) channels.put(channelName, new Channel(channelName));
