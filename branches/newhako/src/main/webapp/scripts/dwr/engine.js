...
dwr.engine._origScriptSessionId = "907B8A316AD6FAE16D9A0EDACD4D78BA";
...
dwr.engine._defaultPath = "/kulsa/dwr";
...
dwr.engine._execute = function(path, scriptName, methodName, vararg_params) {
  ...
  if (callData.path) path = callData.path; 
...
dwr.engine._sendData = function(batch) {
  ...
  if (batch.req) {
    ...
    batch.mode = batch.isPoll ? dwr.engine._ModeScriptPoll : dwr.engine._ModeScriptCall;
    request = dwr.engine._constructRequest(batch);
...
dwr.engine._ModeScriptCall = "/call/scriptcall/";
dwr.engine._ModeScriptPoll = "/call/scriptpoll/";
...
dwr.engine._remoteEndScriptTagResponse = function(batchId) {
  dwr.engine._clearUp(dwr.engine._batches[batchId]);
  dwr.engine._receivedBatch = null;
};
...
dwr.engine._clearUp = function(batch) {
  ...
  // ScriptTag tidyup
  if (batch.script) {
    document.body.removeChild(batch.script);
  }
